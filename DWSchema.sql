...
ADD CONSTRAINT CheckGymWorkers CHECK (
    ...
    [Role] IN ('trainer', 'receptionist', 'manager') AND
    Specialization IN ('cardio', 'strength training', 'dietetics', 'accounting', 'cleaning', 'marketing','human Resources', 'finance', 'PR management') AND
...
ADD CONSTRAINT CheckJunk CHECK (
    TrainingQuality IN ('unknown', '1', '2', '3', '4', '5') AND
    AverageTotalScore IN ('unknown', 'very low', 'low', 'medium', 'high', 'very high')
)

ALTER TABLE SessionTopics
ADD CONSTRAINT CheckTopics CHECK (
    Name IN ('Cardio Boost','Strength Boost','Mixed work-out','Dietetic consultancy','Work-out consultancy','Beginner Boxing','Intermediate Boxing','Profiled stretching','Zumba classes','Slim thighs','Aerobic','Group stretching','Group cardio','Boxing sparring','Group barbells','Trampolines','Group Yoga') AND
...
ADD CONSTRAINT CheckDate CHECK (
    ...
    MonthName IN ('January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December')
)
